...
warnings.filterwarnings(
    ...
    message="You are attempting to write an empty DataFrame to file. For some drivers, this operation may fail.",
)
...
class Submodels:
    ...
    def find_gpkg_file(self):
        ...
        if gpkg_files:
            ...
            raise GeoPackageFileNotFoundError(
                "No GeoPackage file found in the schematisation directory."
            )
    ...
    def find_rasters_directory(self):
        ...
        raise RastersDirectoryNotFoundError(
            "No folder 'rasters' found in the schematisation directory."
        )
        ...
        if path.suffix == ".gpkg":
            try:
                ...
                raise LayerNotFoundError(
                    f"The file at '{path}' does not contain a layer named '{layer_name}'."
                )
        else:
    ...
    def check_field_existence_and_uniqueness(self):
        if self.field_name in self.subareas.columns:
            if self.subareas.empty:
                raise SubareaLayerEmpty(
                    f"The file at '{self.subareas_path}' does not contain any features."
                )
            if self.subareas[self.field_name].duplicated().any():
                raise SubareaNamesNotUnique(
                    f"The values in the '{self.field_name}' column are not unique."
                )
        else:
            raise FieldNameNotFoundError(
                f"The file at '{self.subareas_path}' does not contain a field with the name '{self.field_name}'."
            )
    ...
    def clip(self, subarea):
        ...
        output_schematisation_gpkg_path = output_directory / (
            self.schematisation_gpkg.stem + "_" + name + self.schematisation_gpkg.suffix
        )
        # output_schematisation_sqlite_path = output_directory / (
        # self.schematisation_sqlite.stem
        # + "_"
        # + name
        # + self.schematisation_sqlite.suffix
        # )
        ...
        linear_obstacle = layers_dict.get("obstacle")
        potential_breach = layers_dict.get("potential_breach")
        exchange_line = layers_dict.get("exchange_line")
        grid_refinement = layers_dict.get("grid_refinement_line")
        ...
        filtered_connection_node = self.spatial_join(
            connection_node, subarea_gdf, "inner", "intersects", "_subarea"
        )
        ...
        filtered_pump = pump[pump["connection_node_id"].isin(valid_connection_node_ids)]

        filtered_pipe = pipe[
            (pipe["connection_node_id_start"].isin(valid_connection_node_ids))
            & (pipe["connection_node_id_end"].isin(valid_connection_node_ids))
        ]

        filtered_weir = weir[
            (weir["connection_node_id_start"].isin(valid_connection_node_ids))
            & (weir["connection_node_id_end"].isin(valid_connection_node_ids))
        ]

        filtered_orifice = orifice[
            (orifice["connection_node_id_start"].isin(valid_connection_node_ids))
            & (orifice["connection_node_id_end"].isin(valid_connection_node_ids))
        ]

        filtered_culvert = culvert[
            (culvert["connection_node_id_start"].isin(valid_connection_node_ids))
            & (culvert["connection_node_id_end"].isin(valid_connection_node_ids))
        ]

        filtered_pump_map = pump_map[
            (pump_map["connection_node_id_end"].isin(valid_connection_node_ids))
        ]

        filtered_channel = channel[
            (channel["connection_node_id_start"].isin(valid_connection_node_ids))
            & (channel["connection_node_id_end"].isin(valid_connection_node_ids))
        ]
        ...
        # valid_impervious_surface_ids = filtered_impervious_surface_map[
        ...
        filtered_exchange_line = self.spatial_join(
            temp_exchange_line, subarea_gdf, "inner", "intersects", "_subarea"
        )

        # Filter other items that should be completely within mask
        filtered_lateral_2d = self.spatial_join(
            lateral_2d, subarea_gdf, "inner", "within", "_subarea"
        )
        filtered_boundary_condition_2d = self.spatial_join(
            boundary_condition_2d, subarea_gdf, "inner", "within", "_subarea"
        )
        filtered_potential_breach = self.spatial_join(
            potential_breach, subarea_gdf, "inner", "within", "_subarea"
        )

        # Filter other items that should be atleast partly within mask
        filtered_linear_obstacle = self.spatial_join(
            linear_obstacle, subarea_gdf, "inner", "intersects", "_subarea"
        )
        filtered_grid_refinement = self.spatial_join(
            grid_refinement, subarea_gdf, "inner", "intersects", "_subarea"
        )
        filtered_grid_refinement_area = self.spatial_join(
            grid_refinement_area, subarea_gdf, "inner", "intersects", "_subarea"
        )
        ...
        if self.isolate_1d:
            ...
            # isolated_manhole["calculation_type"] = 1
            isolated_pipe["calculation_type"] = 101
            isolated_culvert["calculation_type"] = 101
            isolated_channel["calculation_type"] = 101

            # concat isolated items manhole, pipe, culvert and channel to the filtered items
            # filtered_manhole = gpd.GeoDataFrame(
            #    pd.concat([filtered_manhole, isolated_manhole], ignore_index=True)
            # )
            filtered_pipe = gpd.GeoDataFrame(
                pd.concat([filtered_pipe, isolated_pipe], ignore_index=True)
            )
            filtered_culvert = gpd.GeoDataFrame(
                pd.concat([filtered_culvert, isolated_culvert], ignore_index=True)
            )
            filtered_channel = gpd.GeoDataFrame(
                pd.concat([filtered_channel, isolated_channel], ignore_index=True)
            )
    ...
    def read_geopackage_layers(self, gpkg_path):
        ...
        for i in range(ds.GetLayerCount()):
            ...
            for j in range(layer_defn.GetFieldCount()):
                ...
                if field_defn.GetNameRef().lower() in ("fid", "ogc_fid", "id"):
                    pk_field = field_defn.GetNameRef()
                    break
            ...
            for feature in layer:
                if pk_field:
                    pk_values.append(feature.GetField(pk_field))
                else:
                    pk_values.append(feature.GetFID())
            layer.ResetReading()
    ...
    def clip_raster(self, input_path, mask_geometry, output_path):
        ...
        with rasterio.open(input_path) as src:
            clipped_data, clipped_transform = mask(src, [mask_geojson], crop=True)
...
def get_parser():
    ...
    parser.add_argument(
        "schematisation_directory",
        help="Required. Folder including .gpkg file and rasters folder.",
    )
